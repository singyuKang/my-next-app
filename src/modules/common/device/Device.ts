...
export abstract class Device {
  ...
  static getDeviceTypeFromHeaders = async () => {
    const headersData = await headers(); // Promise<ReadonlyHeaders> 해제
    const headersObject = Object.fromEntries(headersData); // ReadonlyHeaders를 일반 객체로 변환
  ...
  static getDevice = async () => {
    ...
    if (typeof window === "undefined") {
      deviceType = await this.getDeviceTypeFromHeaders();
    } else {
